...
import {Row,Col,Form,Modal,message,Input,TreeSelect,Select,AutoComplete,Radio,DatePicker,Icon, Cascader,Upload} from 'antd';
import {informSn,informTbox,validatorMobileSpecial,validatorIdNumberSpecial,validatorCarOwnerName} from '../../../util/validator'
import { resolve } from 'url';
import {HttpUrl,httpConfig} from './../../../util/httpConfig'
import  newLeft  from './pic1.png';
import  newRight  from './pic2.png';
import  editLeft  from './pic3.png';
import  editRight  from './pic4.png';
import moment from 'moment';
import {deviceText2,versionCheck} from './../../../util/validator'
...
class addEdit extends Component {
    ...
    componentDidMount(){
        Axios.get(HttpUrl+"appserv/label/getAllLabel",httpConfig).then( res => {
            if(res.data.code === '100000'){
                console.log(res)

                for(let i=0;i<res.data.data.page.length;i++){
                    this.state.optionlabel.push(<Option value={res.data.data.page[i].id} key={i}>{res.data.data.page[i].name}</Option>)
                }
            }
        })
...
//编辑
   ...
   Axios.post(HttpUrl+'appserv/general/getAccountById',{
            'id':record.id,
        }).then(res => {
            console.log(res)
            if(res.data.code === '100000'){
              if(res.data.data.userInfo.userLabel!=null){
                let a=res.data.data.userInfo.userLabel.split(',');
                this.setState({
                        userLabelEdit:res.data.data.userInfo.userLabel.split(',')
                })
             ...
             this.setState({
                  userLabelEdit:this.state.userLabelEdit,
              })
              if(res.data.data.userInfo.idCardPhotoFront){
                this.props.form.setFields({
                  idCardPhotoFront: {
                    value: res.data.data.userInfo.idCardPhotoFront,
                    
                  },
                });
              }
              if(res.data.data.userInfo.idCardPhotoContrary){
                this.props.form.setFields({
                  idCardPhotoContrary: {
                    value: res.data.data.userInfo.idCardPhotoContrary,
                ...
                this.setState({
                      addressInfo1: String(res.data.data.userInfo.province) +"-"+String(res.data.data.userInfo.city) +"-"+String(res.data.data.userInfo.area),
                      editInfo:res.data.data.userInfo,
                      operatData:res.data.data.userInfo.birthdayStr,
                      idCardPhotoFront:res.data.data.userInfo.idCardPhotoFront,
                      idCardPhotoContrary:res.data.data.userInfo.idCardPhotoContrary,
                     
                      sex:res.data.data.userInfo.sex,
                      status:res.data.data.userInfo.status,
                        fileList1:res.data.data.userInfo.idCardPhotoFront?[{
                          uid: '-1',
                          name: '',
                          status: 'done',
                          url:res.data.data.userInfo.idCardPhotoFront
                        }]:[],
                        fileList2:res.data.data.userInfo.idCardPhotoContrary?[{
                            uid: '-2',
                            name: '',
                            status: 'done',
                            url:res.data.data.userInfo.idCardPhotoContrary
                        }]:[],
                       
                    
                })
                console.log(this.state.userLabelEdit)
            }else{
                message.warning(res.data.message)
            }
            console.log(this.state.fileList1,this.state.fileList2)
        })
       this.setState({
        ...
        this.props.form.validateFields((err, values) => {
            ...
            if (!err) {
                if(this.state.operation=='1'){     //新增
                    console.log(14)
                    console.log(this.state.fileList1[0].response.data)
                    console.log(this.state.fileList2[0].response.data)
                    // if(this.state.fileList1[0].response.data){
                    //   message.warning('shengfenz')
                    // }
                    Axios.post(HttpUrl+'appserv/general/addAccount',{
                        'id':this.state.record.id,
                        'name':this.props.form.getFieldValue('name') || null,
                        'mobile':this.props.form.getFieldValue('mobile') || null,
                        'idCard':this.props.form.getFieldValue('idCard') || null,
                        'status':'0',
                        'sex':this.state.sex,
                        'province':this.state.addressInfo[0],
                        'city'    :this.state.addressInfo[1],
                        'area'    :this.state.addressInfo[2],
                        'birthday':this.state.operatData,
                        'address':this.props.form.getFieldValue('address') || null,
                        'idCardPhotoFront':this.state.fileList1[0].response.data,
                        'idCardPhotoContrary':this.state.fileList2[0].response.data,
                        'realNameAudit':this.state.fileList2[0].response.data&&this.state.fileList1[0].response.data?'1':'0',
                    ...
                    if(this.state.fileListEdit1.length>0){
                      console.log(23)
                      this.setState({
                        idCardPhotoFront:this.state.fileListEdit1[0].response.data
                      })
                    }else{
                      this.setState({
                        idCardPhotoFront:''
                    ...
                    if(this.state.fileListEdit2.length>0){
                      console.log(234)
                      this.setState({
                        idCardPhotoContrary:this.state.fileListEdit2[0].response.data
                      })
                    }else{
                      this.setState({
                        idCardPhotoContrary:''
                    ...
                    console.log(200)
                        Axios.post(HttpUrl+'appserv/general/updateAccount',{
                          'id':this.state.record.id,
                          'name':this.props.form.getFieldValue('name') || null,
                          'mobile':this.state.editInfo.mobile,
                          'idCard':this.props.form.getFieldValue('idCard') || null,
                          'status':this.state.status,
                          'sex':this.state.sex,
                          'province':this.state.editInfo.province,
                          'city'    :this.state.editInfo.city,
                          'area'    :this.state.editInfo.area,
                          'birthday':this.state.operatData,
                          'address':this.props.form.getFieldValue('address') || null,
                          'idCardPhotoFront':this.state.fileListEdit1.length>0?this.state.fileListEdit1[0].response.data:this.state.idCardPhotoFront,
                          'idCardPhotoContrary':this.state.fileListEdit2.length>0?this.state.fileListEdit2[0].response.data:this.state.idCardPhotoContrary,
                          'realNameAudit':this.state.fileList2&&this.state.fileList1?'1':'0',
                          'userLabel':this.state.userLabel,
                          'realNameAudit':this.state.idCardPhotoFront&&this.state.idCardPhotoContrary?'1':'0',
                          'birthday':this.state.operatData!=''?this.state.operatData:null
                        }).then(res => {
                            console.log(res)
                            if(res.data.code === '100000'){
                                this.setState({
    ...
    getBirthday=(e,)=>{
      ...
      if(e.target.value){
        let a=e.target.value.substring(6,10)+'-'+e.target.value.substring(10,12)+'-'+e.target.value.substring(12,14)
        this.setState({
          operatData:a
        })
      }
    ...
    selectChange=(value)=>{
        ...
        for(let i=0;i<value.length;i++){
             labelInfo= labelInfo+','+value[i];

        }
        labelInfo=labelInfo.substr(1,labelInfo.length-1);
               ...
               <Modal
                ...
                <Row>
                    <Col span={24}>
                        ...
                        <div  className="addForm" style={{marginTop:28,marginLeft: 18}}>
                        <Form>
                                <FormItem className="form_input" label="车主姓名：" labelCol={{span:5}} wrapperCol={{span:19}} style={{}}>
                                    { getFieldDecorator('name',{
                                        rules: [
                                            { required: true ,validator:validatorCarOwnerName,}
                                        ],
                                        initialValue:this.state.operation=='2' ? this.state.editInfo.name : ''
                                    })(<Input  onKeyDown={this.keycode} autoComplete='off' type="text"  style={{textTransform:'uppercase',width:200}} />)}
                                </FormItem>
                               { this.state.operation=='1'?
                                <FormItem className="form_input" label="手机号(账号)：" labelCol={{span:5}} wrapperCol={{span:19}} style={{}}>
                                    { getFieldDecorator('mobile',{
                                        rules: [
                                            { required: true ,validator:validatorMobileSpecial}
                                        ],
                                    })(<Input  onKeyDown={this.keycode} autoComplete='off' type="text"  style={{textTransform:'uppercase',width:200}} />)}
                                </FormItem>:
                                <FormItem className="form_input" label="手机号(账号)：" labelCol={{span:5}} wrapperCol={{span:19}} style={{}}>
                                         <span>{this.state.editInfo.mobile}</span>
                                </FormItem>}
                                <FormItem className="form_input" label="身份证号：" labelCol={{span:5}} wrapperCol={{span:19}} style={{}}>
                                    { getFieldDecorator('idCard',{
                                        rules: [
                                            { required: true ,validator:validatorIdNumberSpecial}
                                        ],
                                        initialValue:this.state.operation=='2' ? this.state.editInfo.idCard : ''

                                    })(<Input onChange={this.getBirthday} onKeyDown={this.keycode} autoComplete='off' type="text"  style={{textTransform:'uppercase',width:200}} />)}
                                </FormItem>
                               <FormItem  className="form_input" label="出生日期：" labelCol={{span:5}} wrapperCol={{span:19}} >
                                   
                                      <div style={{border:'1px solid #d9d9d9',height:28,width:200}}>
                                       <div style={{marginTop:-7,marginLeft:10}}><span>{this.state.operatData}</span></div> 
                                      </div>
                                </FormItem>
                                   
                                <FormItem className="form_input" label="车主性别：" labelCol={{span:5}} wrapperCol={{span:19}} >
                                        <RadioGroup name="radiogroup" onChange={this.sexChange} defaultValue={this.state.operation=='2' ? this.state.editInfo.sex=='0'?0:1 : 0} >
                                            <Radio value={0}>男</Radio>
                                            <Radio value={1}>女</Radio>
                                        </RadioGroup>
                                </FormItem>
                                { this.state.operation=='1'?   
                                <FormItem  className="form_input" label="注册地址" labelCol={{span:5}} wrapperCol={{span:19}} >
                               { getFieldDecorator('idCardaddress',{
                                ...
                                <FormItem  className="form_input" label="注册地址" labelCol={{span:5}} wrapperCol={{span:19}}  >
                                    <span>{this.state.addressInfo1}</span>
                               </FormItem>}
                                <FormItem  className="form_input" label="详细地址：" labelCol={{span:5}} wrapperCol={{span:19}} >
                                    {getFieldDecorator('address',{
                                       initialValue:this.state.operation=='2' ? this.state.editInfo.address : ''

                                    })(<Input  onKeyDown={this.keycode} autoComplete='off' type="text" maxLength={17} style={{textTransform:'uppercase',width:360}} />)}
                                </FormItem>
                            
                                <FormItem  className="form_input formSelect" label="用户标签：" labelCol={{span:5}} wrapperCol={{span:19}}  style={{}}>
                                    <Select  showSearch
                                        mode="multiple"
                                        optionFilterProp="children"
                                        onChange={this.selectChange}
                                        defaultValue={this.state.operation=='2' ?this.state.userLabelEdit:undefined}
                                        style={{width:360,}}
                                ...
                                <FormItem  className="form_input" label="身份证照片：" labelCol={{span:5}} wrapperCol={{span:19}} style={{}}>
                                   
                                </FormItem>
                        </Form>
                      
                        <div style={{height:160,textAlign:'center',marginLeft:42,}}>  
                        <Col span={12}>
                         <FormItem className="form_input"> 
                                         { getFieldDecorator('idCardPhotoFront',{
                                                rules: [
                           ...
                           <Upload
                                action={HttpUrl+"appserv/uploadFile/uploadIdCardImage"}
                                headers= {token={token} }
                            ...
                            >
                                {this.state.fileList1.length >= 1 ? null :this.state.operation=='1'? uploadButton2:uploadButton3}
                            </Upload>
                            </div> 
                             )} 
                                    </FormItem> 
                            <span><span style={{color:'#FF3636'}}>* </span>请上传身份证正面照片</span>
                        </Col>
                        <Col span={12}>
                        <FormItem className="form_input"> 
                                        { getFieldDecorator('idCardPhotoContrary',{
                            ...
                            >
                                {this.state.fileList2.length >= 1 ? null : this.state.operation=='1'? uploadButton1:uploadButton4}
                            </Upload>
                            </div>
                             )} 
                         </FormItem> 
                            <span><span style={{color:'#FF3636'}}>* </span>请上传身份证反面照片</span>
                        </Col>
                ...
                <style>
                    {`
                    .picCard .ant-upload-list-picture-card .ant-upload-list-item-actions .anticon-eye-o, .ant-upload-list-picture-card .ant-upload-list-item-actions .anticon-delete{
                      font-size: 24px;
                       width: 24px;
                    }
                    .picCard .ant-upload-list-picture-card .ant-upload-list-item-actions .anticon-eye-o{
                      display:none;
                    }
                    .picCard .ant-upload-list-picture-card .ant-upload-list-item-info:before{
                      position: absolute;
                      left: 0px;
                    }
                    .picCard .ant-upload.ant-upload-select-picture-card{
                      border:none;
                    }
                    .picCard  .ant-upload-list-picture-card .ant-upload-list-item{
                       padding:0px;
                    }
                    .picCard .ant-upload.ant-upload-select-picture-card > .ant-upload{
                      padding:0px;
                    }
                    .formSelect .ant-select-selection{
                      height:100%;
                    }
                    .picCard .ant-upload-list{
                      border:none;
                      height:0px;
                    }
                    .picCard .ant-upload-list-picture-card .ant-upload-list-item{
                    ...
                    .addForm .ant-form-item{margin-bottom:10px;}
                    .picCard{height:100px;width:180px; margin-left:28px;margin-bottom:40px;}
                    ...
                    .editModel{width:700px!important;height:700px!important;}
                    .editModel .ant-steps{width:90%!important;padding-left:5%}
                    .addModel{width:539px!important;font-size:13px;}
