...
class distribution extends Component {
    ...
    //统计
    tongji = (obj) => {
        return  Axios.post(HttpUrl+'data/power/statistics',{
    ...
    search = () => {
        new Promise(resolve => {
            resolve(Axios.post(HttpUrl+'data/power/select',{
                ...
                if(res.data.code === '100000'){
                    this.setState({
                        ...
                        totalSum2:res.data.data.sum.toFixed(2)
                    })
                    return true
                }else{
                    errorCallback(res.data.code)
    ...
    render() {
        ...
        <div className="content statistic" >
            <Tabs type="card" onChange={ this.tabChange} >
                ...
                <Common wrappedComponentRef={(form) => this.form = form} whatTime='short' orShowTime='false,false' onRef={this.onRef} btnList={this.state.btnList.includes('powerListA')}></Common>
                </TabPane>
                    ...
                    <div  className='content-title'>
                        <Form layout="inline">  
                            <div className='searchType' style={{width:'100%'}}>
                                ...
                                {
                                    this.state.btnList.includes('powerListB') ?
            ...
            <div>
                <div  className='oprateHead'>
                {
                    this.state.tabKey === '1' ?
                    (
                        this.state.btnList.includes('powerExportA') ?
                    ...
                    :(
                        this.state.btnList.includes('powerExportB') ?
                        <Button type="primary" className='btn' onClick={ this.export} ghost>
                            <img src={exports} alt="" />
                            导出
                        </Button>
               ...
               <div className='table' style={{border:'1px solid #EBEDF8',padding:'34px 0px',background:"#fff"}}>
                    ...
                    <div style={{display:this.state.tabKey === '2' ? 'block' : 'none'}}>
                        ...
                        <div style={{width:'95%',textAlign:'right',marginTop:'20px'}}>总耗电量：{this.state.totalSum2}km</div>
                    </div>
               </div>
            </div>
            <style>
