...
class noticeModify extends Component {
    ...
    state = {
        ...
        notice: {
            ...
            noticePushNow: false,
            selectTime: '2019-01-01',   // 及时通知不是立即发送，则必选择发送时间,最多选择 5 个，中间以‘;’分隔,请去掉最后末尾得分号
            ...
            week: 'MON',                // 每周的星期几发送，定时触发的触发条件为 2（即每周）时，必填此项，值为：
    ...
    showModal = (record) => {
        this.setState({
            show: true,
            // 需要处理一下记录
            notice: Object.assign(this.state.notice, record, {
                noticePushNow: record.differentiate === 1 ? false : true,
            })
        });
    }
    ...
    addNoticeSelectTimeItem = () => {
        ...
        const nextKeys = keys.concat(++state.selectTimeId);
        form.setFieldsValue({
            keys: nextKeys,
        });
    ...
    // ====================
    // 12.4消息通知管理修改
    // 请求方式：POST;接口路径：http://42.159.92.113/api/appserv/push/updatePush
    addMessage = () => {
        let params = {
            // 已有字段
        ...
        return new Promise((resolve, reject) => {
            Axios.post(HttpUrl + `appserv/push/updatePush`, { ...params }, httpConfig)
                .then((res) => {
    ...
    updateNotice = () => {
        return new Promise((resolve, reject) => {
            ...
            Axios.post(HttpUrl + `appserv/push/updatePush`, { ...this.state.notice }, httpConfig)
                .then((res) => {
                    this.setState({ loading: false });
                    if (res.data.code === '100000') {
                        resolve(res.data);
    ...
    render() {
        ...
        const formItemLayoutNoticeLabel = { wrapperCol: { xs: { span: 24, offset: 0 }, sm: { span: 18, offset: 6 }, }, };
        ...
        return (
            <div className="notice notice_add" >
                ...
                <Modal
                    ...
                    onOk={this.submit}
                    title={state.notice.pushType === 1 ? `${state.title}及时通知` : `${state.title}固定通知`}
                    ...
                    <Form className="notice_from" style={{ height: '480px', overflowY: 'auto', overflowX: 'hidden' }}>
                        <Form.Item {...formItemLayout} label="消息类型">
                            {getFieldDecorator('pushType')(
                                <span>{state.notice.pushType === 1 ? '及时消息' : '固定消息'}</span>
                            )}
                        </Form.Item>
                        
                        <Form.Item {...formItemLayout} label="消息标题">
                            {getFieldDecorator('title', {
                                initialValue: state.notice.title,
                                rules: [
                        ...
                        {/* 推送渠道 */}
                        <Form.Item {...formItemLayout} label="推送渠道" defaultValue={state.notice.way}>
                            {getFieldDecorator('way', {
                                initialValue: state.notice.way * 1 === 1 ? 'App' : '短信',
                                rules: [{
                        ...
                        {state.notice.saveMessageCenter &&
                            <Form.Item {...formItemLayout} label="消息位" defaultValue={0}>
                                {getFieldDecorator('messageDigit', {
                                    ...
                                    <Select onChange={this.noticeMessageDigitChange} style={{ width: 200 }} >
                                        {
                                            state.select.messageDigitSelect.map(
                        ...
                        {/* 推送时间 */}
                        {state.notice.pushType === 1 && <Form.Item {...formItemLayout} label="推送时间" >
                            {getFieldDecorator('differentiate', {
                                initialValue: state.notice.noticePushNow,
                            })(
                                <Checkbox onChange={this.noticeDifferentiateChange} checked={state.notice.noticePushNow}>
                                    立即发送
                                </Checkbox>
                            )}
                        </Form.Item>}
                        ...
                        {/* 选择推送时间 */}
                        {state.notice.pushType === 1 && !state.notice.noticePushNow && 
                        ...
                        {state.notice.pushType === 1 && !state.notice.noticePushNow && formTimeItems}

                        {/* 触发方式 */}
                        {state.notice.pushType === 2 && <div>
